...
import { requireRole, requireRoleOrSelf } from "../middlewares/roleMiddleware.js";
...
router.get("/", authMiddleware, requireRole(["admin", "support1"]), userController.getAllUsers);

// Get user profile by ID (admin only)
router.get("/:userId", authMiddleware, requireRoleOrSelf(["admin"]), userController.getUserById);

// Update user profile
router.put("/profile", authMiddleware, userController.updateUserProfile);

// Update user profile by ID (admin only)
router.put("/:userId", authMiddleware, requireRole(["admin"]), userController.updateUserProfileById);
