...
2. `POST /api/auth/refresh` returns `{ authenticated: true }`.
3. Protected browser auth routes (`/api/auth/logout`, `/api/auth/me`, `/api/auth/admins`) require cookie access context.
4. Refresh token source is cookie-only for `/api/auth/refresh`.
5. Browser state-changing endpoints (`POST`, `PUT`, `PATCH`, `DELETE`) require valid `X-CSRF-Token` matching CSRF cookie context; missing/invalid token returns `403 CSRF_INVALID`.
6. Legacy compatibility flags are removed from runtime and config parsing.
